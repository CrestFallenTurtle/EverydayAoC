...
class Mult:
  ...
  def __init__(self) -> None:
    self.function_name = "mult"

  def start(self, arguments:list[str], var_namespace:dict[str, any]) -> None:

    if len(arguments) != 3:
      error(f"The wrong amount of arguments where sent in, expected three, got {len(arguments)}")
    ...
    try:
      ...
      error(f"Failed to convert one or more of the provided arguments into an integer\n{err}")
